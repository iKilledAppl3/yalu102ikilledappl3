...
//shows device info from app
//@iKilledAppl3
@property (retain, nonatomic) IBOutlet UILabel *deviceVersionLabel;
@property (retain, nonatomic) IBOutlet UILabel *deviceTypeLabel;
...
- (void)viewDidLoad {
    ...
    if (strstr(u.version, "deviceARM128")) {
        [_freeMe setEnabled:NO];
        [_freeMe setTitle:@"I'm free!" forState:UIControlStateDisabled];
    ...
    self.deviceVersionLabel.text = sysVers;
...
gotclock:;
    ...
    *(uint64_t*)(((uint64_t)faketask) + 0x2b8) = itk_kern_sself;
    ...
    [_freeMe setEnabled:NO];
    [_freeMe setTitle:@"already jailbroken" forState:UIControlStateDisabled];
...
- (void)dealloc {
    [_deviceVersionLabel release];
    [_deviceTypeLabel release];
    [_freeMe release];
    [super dealloc];
}
